...
export function getLocationMetaDescription(placeName, countrySuffix = '') {
  return `Independent HP printer support in ${placeName}${countrySuffix}. Fix offline errors, paper jams, driver issues, WiFi problems, and error codes on LaserJet, OfficeJet, DeskJet, and Envy — available 24/7.`;
}

export function getLocationKeywords(placeName, countrySuffix = '') {
  const country = countrySuffix ? ' Canada' : ' USA';
  return `HP printer support ${placeName}, HP printer help ${placeName}, fix HP printer error ${placeName}${country}`;
}
...
export function generateHpLocationContent({
  ...
  return `
    ...
    <p>Need independent HP printer support in ${inPlace}? Our technicians help with LaserJet, OfficeJet, DeskJet, and Envy printers — offline errors, paper jams, driver failures, WiFi problems, and error codes.</p>
