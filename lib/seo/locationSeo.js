...
export function getLocationPageTitle(placeName) {
  return `HP Printer Help in ${placeName}`;
}

export function getLocationMetaDescription(placeName, countrySuffix = '') {
  return `Need HP printer help in ${placeName}${countrySuffix}? We troubleshoot offline printers, paper jams, driver errors, and WiFi issues remotely. Available 24/7 for LaserJet, OfficeJet, DeskJet, and Envy.`;
}

export function getLocationKeywords(placeName, countrySuffix = '') {
  const country = countrySuffix ? ' Canada' : ' USA';
  return `HP printer help ${placeName}, HP printer repair ${placeName}, HP printer support ${placeName}${country}`;
}
...
export function generateHpLocationContent({
  ...
  return `
    <h1>HP Printer Help in ${placeName}</h1>
    <p>Need printer support in ${inPlace}? Whether you use a LaserJet, OfficeJet, DeskJet, or Envy, our certified technicians provide fast remote help for jams, offline errors, print quality issues, driver problems, and WiFi setup.</p>

    ${getLocalIntroHtml(slug, placeName, areasMap, countrySuffix)}

    <h2>Why Choose Us in ${placeName}?</h2>
    <p>We serve residents and businesses across ${placeName}. Most issues are diagnosed and fixed remotely — no shop visit required — so you can print again quickly.</p>
    ...
    <h3>Get Help in ${placeName} Today</h3>
    <p>Don't let a broken printer slow you down in ${placeName}. Contact our team:</p>
    <ul>
      <li>Call us at <a href="tel:+18884237757">(888) 423-7757</a></li>
      <li>Available 24/7 for setup, repair, and troubleshooting</li>
    </ul>

    <p>From LaserJet paper jams to OfficeJet WiFi issues, we're here to help anywhere in ${inPlace}.</p>
  `;
}
