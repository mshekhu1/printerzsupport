...
import { HP_MODELS } from './siteSeo';

const LOCATION_META_VARIANTS = [
  (place, suffix) =>
    `Authorised repair for HP printers in ${place}${suffix}. Get remote support for offline errors, jams, drivers, and WiFi on ${HP_MODELS}.`,
  (place, suffix) =>
    `Authorised HP repair in ${place}${suffix}, available 24/7. Printer specialists support offline fixes, jam removal, drivers, and WiFi.`,
  (place, suffix) =>
    `Authorised HP repair specialists serving ${place}${suffix}. Get printer support for ${HP_MODELS}, including drivers, WiFi, and offline errors.`,
  (place, suffix) =>
    `Authorised servicing for HP devices in ${place}${suffix}. Printer support is available 24/7 for error codes, jams, drivers, and wireless setup.`,
];

const LOCATION_KEYWORD_VARIANTS = [
  (place, country) => `HP printer authorised repair ${place}, fix HP printer ${place}${country}`,
  (place, country) => `24/7 customer service ${place}, HP printer help ${place}${country}`,
  (place, country) => `HP printer repair service ${place}, HP technicians ${place}${country}`,
  (place, country) => `HP printer servicing ${place}, HP repair help ${place}${country}`,
];
...
export function getLocationPageTitle(placeName) {
  return `HP Printer Authorised Repair in ${placeName}`;
}

export function getLocationMetaDescription(placeName, countrySuffix = '', slug = '') {
  const variant = hashSlug(slug || placeName) % LOCATION_META_VARIANTS.length;
  return LOCATION_META_VARIANTS[variant](placeName, countrySuffix);
}

export function getLocationKeywords(placeName, countrySuffix = '', slug = '') {
  const country = countrySuffix ? ' Canada' : ' USA';
  const variant = hashSlug(slug || placeName) % LOCATION_KEYWORD_VARIANTS.length;
  return LOCATION_KEYWORD_VARIANTS[variant](placeName, country);
}
...
export function generateHpLocationContent({
  ...
  return `
    <h1>HP Printer Authorised Repair in ${placeName}</h1>
    <p>Need authorised repair for an HP device in ${inPlace}? Our printer specialists provide remote support for ${HP_MODELS} — offline errors, paper jams, driver failures, and WiFi problems.</p>
    ...
    <p>Comprehensive repair and printer support for HP owners in ${placeName}:</p>
    <ul>
    ...
    <h3>Book HP Printer Repair in ${placeName}</h3>
    <p>Call <a href="tel:+18884237757">(888) 423-7757</a> — 24/7 repair assistance for setup, faults, and troubleshooting.</p>
